...
describe PolyTreeNode do
  ...
  describe "#parent=" do
    before do
      node2.parent = node1 # if self.parent ==  parent
      node3.parent = node1
    end

    it "should set a node's parent to the passed node" do
      expect(node2.parent).to equal(node1) #self.parent = parent 
      expect(node3.parent).to equal(node1)
    end

    it "should add the child node to the passed node's children" do
      expect(node1.children).to eq([node2, node3]) #parent.children << self
    end
    ...
    it "handles nil without issue" do
      node2.parent = nil  #self.parent = nil 
